    ...
    Connections {
        ...
        function onCurrentTrackChanged() {
            if (DBApi.playback.stopped) {
                windo.title = DBApi.playback.tf_current(conf.get("MainWindow", "titlebar_stopped_tf", "DeaDBeeF"))
            }
            else {
                windo.title = DBApi.playback.tf_current(conf.get("MainWindow", "titlebar_playing_tf", "DeaDBeeF - %artist% - %title%"))
            }
        }
        ...
        Page {
            ...
            footer: RowLayout {
                ...
                Label {
                    ...
                    text: (DBApi.playback.playing ? "Playing" : DBApi.playback.paused ? "Paused" : "Stopped") + " " +
                          "Pos: " + curr.toFixed(1) + ", Track Length: " + DBApi.playback.position_max.toFixed(1) + "s"
                    Timer {
                        interval: 10
                        running: true
                        repeat: true
                        onTriggered: {
                            pos.curr = DBApi.playback.position_rel
