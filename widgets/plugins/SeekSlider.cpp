...
SeekSlider::SeekSlider(QWidget *parent, DBApi *Api) : QSlider(parent) {
    ...
    //connect(, SIGNAL(playbackStarted()),this,SLOT(onPlaybackStart()));
...
QObject *SeekSlider::constructor(QWidget *parent, DBApi *Api) {
    if (!parent) {
        QObject *info = new QObject(nullptr);
        info->setProperty("friendlyName", info->tr("Seek Slider"));
        info->setProperty("internalName", "seekSlider");
...
void SeekSlider::mousePressEvent ( QMouseEvent * event ) {
    ...
    if ((event->button() == Qt::LeftButton || event->button() == Qt::RightButton) && sr.contains(event->pos()) == false) {
        ...
        if (orientation() == Qt::Vertical) {
            ...
            newVal = minimum() + ((maximum()-minimum()) * (height()-event->position().y()) / height());
#else
...
#if QT_VERSION > QT_VERSION_CHECK(6,0,0)
            int adaptedPosX = event->position().x();
#else
...
void SeekSlider::onFrameUpdate() {
    ...
    if (isHidden() || parentWidget()->isHidden())
        return;
    ddb_playback_state_t output_state = (ddb_playback_state_t) api->playback.getState();
...
#if QT_VERSION > QT_VERSION_CHECK(6,0,0)
    int val = ((float)ev->position().x() / this->width()) * maximum();
#else
