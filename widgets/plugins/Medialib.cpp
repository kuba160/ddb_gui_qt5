...
MedialibTreeView::MedialibTreeView(QWidget *parent, DBApi *Api) : QTreeView(parent) {
    ...
    // Model
    ms_model = api->playlist.createMediasource(this, CONFGET("medialib_plugin", "medialib").toString(), this->property("internalName").toString());
    //prox_model = new MedialibSorted(this, ms_model->getSourcePlugin());
    //prox_model->setSourceModel(ms_model);
    setModel(ms_model);//prox_model);
...
void MedialibTreeView::onModelReset() {
    expand(ms_model->index(0,0,QModelIndex()));
...
void MedialibTreeView::onSearchQueryChanged(QString str) {
    ...
    //     QModelIndex i = sel[0];
    //     while (i.isValid()) {
    //         QString n = ms_model->getSourcePlugin()->tree_item_get_text((ddb_medialib_item_t*) i.internalPointer());
...
void MedialibTreeView::showContextMenu(QPoint p) {
    ...
    //     }
    // }
    qDebug() << "MedialibTreeView: TODO: context menu";
...
Medialib::Medialib(QWidget *parent, DBApi *Api) {
    // Tree setup
    ...
    // Setup selectors
    search_query->addItems(tree->ms_model->getPresets());
    ...
    tree->ms_model->setPresetIdx(search_query_curr);
...
QObject *Medialib::constructor(QWidget *parent, DBApi *api) {
    if (!parent) {
        QObject *info = new QObject(nullptr);
        info->setProperty("friendlyName", info->tr("Media library"));
        info->setProperty("internalName", "medialib");
...
void Medialib::onSelectorChanged(int sel) {
    if (sel < tree->ms_model->getPresets().length()) {
        tree->ms_model->setPresetIdx(sel);
